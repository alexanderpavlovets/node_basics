...
cli.responders.listLogs = function() {
  ...
  ls.stdout.on('data', function(dataObject) {
    ...
    logFileNames.forEach(function(logFileName) {
      if(typeof(logFileName) === 'string' && logFileName.length > 0 && logFileName.indexOf('-') > -1) { // indexOf('-') is for only compressed logs
        console.log(logFileName.trim().split('.')[0]) // log and remove extension
        cli.verticalSpace()
      }
    })
  })
